...
class RectangleRegion: 
    ...
    def transform_to_local(self, other) -> None: # transform self to local coordinates of other
        self.top_left -= other.top_left
        self.bottom_right -= other.top_left
    
    def intersection(self, other): # returns the intersection of two rectangles
...
class Slicer:
    def __init__(self, imgs_path, labels_path, output_path, nn_input_size, empty_img_ratio = None, bbox_size_threshold = None, sliced_bbox_operation = Sliced_BBox_Operation.DELETE, shuffle_empty = False): 
    ...
    def calculate_slice_regions(self, image_height, image_width, target_height, target_width, overlap_height_ratio, overlap_width_ratio):
    ...
    def _save_labels(self, sliced_labels, name):
    ...
    def resize_slice(self, img_slice, target_long_side):
    ...
    def _slice_and_save_imgs(self, regions, img, name, ext) -> None:
    ...
    def _process(self, img_basename, target_height, target_width, overlap_height_ratio, overlap_width_ratio):
    ...
    def slice(self, target_height, target_width, overlap_height_ratio, overlap_width_ratio, workers = os.cpu_count()):
        start = time.time()
    ...
    def single_slice(self, img_basename,  target_height, target_width, overlap_height_ratio, overlap_width_ratio):
        self._process(img_basename, target_height, target_width, overlap_height_ratio, overlap_width_ratio)

    
    def _calc_slice_labels(self, image_height, image_width, regions, labels): 
        new_regions = copy.copy(regions) # doesnt need to use deepcopy since del only deletes references
        new_labels = {region_index: [] for region_index in regions}
    ...
    def _filter_empty_labels(self, regions, labels): # takes in pointers and deletes items from the list/dict
